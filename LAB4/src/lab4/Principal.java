...
public class Principal {
	...
	public static void main(String[] args) {
		...
		for(i=0; i < listaGeral.size(); i++ ) {
			if( listaGeral.get(i) instanceof Aluno) {
				((Aluno)listaGeral.get(i)).imprimeDados();
			}else if ( listaGeral.get(i) instanceof Professor ) {
				((Professor)listaGeral.get(i)).imprimeDados();
			}else {
				((Funcionario)listaGeral.get(i)).imprimeDados();
			}
		}
	}
}
